...
SkipList::SkipList()
{
    head = new SkipNode();
    tail = new SkipNode();
    head->nodePointers[0].prev = NULL;
    head->nodePointers[0].next = tail;
    tail->nodePointers[0].prev = head;
    tail->nodePointers[0].next = NULL;
...
void SkipList::insert(int key, HSLAPixel value)
{
    ...
    maxLevel = max(newNodeLevel, maxLevel);

    while (forwardLevel < newNodeLevel)
    {
        if(forward == tail)
        {
    ...
    while (backwardLevel < newNodeLevel)
    {
        if(prev == head)
        {
...
SkipNode * SkipList::findRHelper(int key, int level, SkipNode * curr)
{
    ...
    if (curr == tail || level < 0) return NULL;
 ...
 * @param key The key to search for
 * @return A pointer to the node with the given
 *  key, or NULL if it could not be found
 */
