...
Graph PremadeGraphs::createChinaMap(bool isWeighted){
    ...
    size_t numCities = 10;
    //Creates a map of some Chinese cities.
    string labels[] = {"Beijing", "Changchun", "Chengdu", "Chongqing", "Guangzhou",
                       "Hangzhou", "Hong Kong", "Shanghai", "Shenyang", "Wuhan"};
    vector<Vertex> vertices(numCities);
    for (size_t i = 0; i < numCities; ++i) {
        vertices[i] = g.insertVertex(labels[i]);
        g.setVertexName(i, labels[i]);
    ...
    g.insertEdge(vertices[8], vertices[0]);
    g.insertEdge(vertices[0], vertices[9]);
    g.insertEdge(vertices[9], vertices[3]);
    g.insertEdge(vertices[3], vertices[2]);
    g.insertEdge(vertices[9], vertices[4]);
    g.insertEdge(vertices[4], vertices[6]);
    g.insertEdge(vertices[9], vertices[5]);
    g.insertEdge(vertices[5], vertices[7]);

    if(isWeighted){
        //g.setEdgeWeight(vertices[0], vertices[1], 912);
        /* g.setEdgeWeight(vertices[0], vertices[2], 1515);
        g.setEdgeWeight(vertices[0], vertices[3], 1869);
        g.setEdgeWeight(vertices[0], vertices[4], 1874); */
        //g.setEdgeWeight(vertices[0], vertices[5], 1329);
        /* g.setEdgeWeight(vertices[0], vertices[6], 2500);
        g.setEdgeWeight(vertices[0], vertices[7], 1213); */

        
        /* g.setEdgeWeight(vertices[1], vertices[2], 1200);
        g.setEdgeWeight(vertices[1], vertices[3], 1200);
        g.setEdgeWeight(vertices[1], vertices[4], 1200);
        g.setEdgeWeight(vertices[1], vertices[5], 1200);
        g.setEdgeWeight(vertices[1], vertices[6], 1200);
        g.setEdgeWeight(vertices[1], vertices[7], 1200); */
        g.setEdgeWeight(vertices[1], vertices[8], 310);
         g.setEdgeWeight(vertices[0], vertices[8], 696);
         g.setEdgeWeight(vertices[0], vertices[9], 1169);
         g.setEdgeWeight(vertices[3], vertices[9],873 );
        g.setEdgeWeight(vertices[2], vertices[3], 302);
        g.setEdgeWeight(vertices[4], vertices[9], 978);
        g.setEdgeWeight(vertices[4], vertices[6], 145);
        g.setEdgeWeight(vertices[5], vertices[9], 728);
        g.setEdgeWeight(vertices[5], vertices[7], 173);
       // g.setEdgeWeight(vertices[2], vertices[4], 1200);
        //g.setEdgeWeight(vertices[2], vertices[5], 1846);
        //g.setEdgeWeight(vertices[2], vertices[6], 1200);
        //g.setEdgeWeight(vertices[2], vertices[7], 1200);
        //g.setEdgeWeight(vertices[2], vertices[8], 1200);
        //g.setEdgeWeight(vertices[3], vertices[9],873 );
        /* g.setEdgeWeight(vertices[3], vertices[4], 1200);
        g.setEdgeWeight(vertices[3], vertices[5], 1200);
        g.setEdgeWeight(vertices[3], vertices[6], 1200);
        g.setEdgeWeight(vertices[3], vertices[7], 1200); */
