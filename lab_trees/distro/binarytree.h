...
class BinaryTree
{
  ...
  private:
    ...
    void outMirror(Node* subRoot);
    bool check_L_to_R(Node* subRoot) const;
    void printPaths(Node* subRoot,vector<vector<T> > &paths,vector<T> path) const;
    int  sumDistances(int curr_height,Node* subRoot) const;
};
