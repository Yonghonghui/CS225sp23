...
class KDTree
{
  ...
  private:
    ...
    vector<Point<Dim>> _fastSort(vector<Point<Dim>>& points, int dim);

    /* Helper function for constructor (building vector) */
    vector<Point<Dim>> _KDTree(vector<Point<Dim>>& points, int dim);

    /* Helper function for constructor (building tree)*/
    KDTreeNode* _buildTree(const vector<Point<Dim>>& points, int start, int end);

    /* Helper function for findNearestNeighbor*/
    KDTreeNode* _findNearestNeighbor(const Point<Dim>& target, KDTreeNode* curr, int dim) const;

    /* Helper function for copy */
    KDTreeNode* _copy(KDTreeNode* curr);

    /* Helper function for delete*/
    void _delete(KDTreeNode* curr);
};
