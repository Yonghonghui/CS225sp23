...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1Upper.begin(), word1Upper.end(), word1Upper.begin(), ::toupper);
    std::transform(word2Upper.begin(), word2Upper.end(), word2Upper.begin(), ::toupper);
    if(dict.find(word1Upper) == dict.end() || dict.find(word2Upper) == dict.end()){
        return false;//one or both words weren't in the dictionary
    }
    vector<string> word1Pronun = dict.find(word1Upper)->second;
    vector<string> word2Pronun = dict.find(word2Upper)->second;
    if(word1Pronun == word2Pronun){
        return true;
    }
    return false;
}
