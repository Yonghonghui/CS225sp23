    ...
    for(int i=0; i<theSource.getRows(); i++){
        for(int j=0; j<theSource.getColumns(); j++){
            ...
            nearest = TilesTree->findNearestNeighbor(real);
                 ...
                 if(result->tile_k_map.find(TilesPointsMap[nearest]) == result->tile_k_map.end()){                     
                pair <unsigned short,TileImage*> pair1 (k, TilesPointsMap[nearest]);                               
                pair <TileImage*,unsigned short> pair2 (TilesPointsMap[nearest], k);                              
                result->k_tile_map.insert(pair1);                                                               
                result->tile_k_map.insert(pair2);                                                                
                k++;                                                                                         
            }                                                                                                  
            result->setTile(i, j, result->tile_k_map.find(TilesPointsMap[nearest])->second);    
