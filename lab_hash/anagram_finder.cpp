...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    if (word.length() != test.length())
        return false;
    Dict<char, int> hashTable_word(300);
    Dict<char, int> hashTable_test(300);
    for (size_t i = 0; i < word.length(); i++) {
        if (hashTable_word.keyExists(word[i]))
            hashTable_word[word[i]]++;
        else
            {hashTable_word.insert(word[i], 1);}
        if (hashTable_test.keyExists(test[i]))
            hashTable_test[test[i]]++;
        else
            {hashTable_test.insert(test[i], 1);}
    }
    for (size_t i = 0; i < word.length(); i++) {
        if (!hashTable_test.keyExists(word[i]))
            return false;//if the letter is not in the test word
        if (hashTable_word[word[i]] != hashTable_test[word[i]])
            return false;//if the letter is in the test word but the number of times it appears is different
    }
    return true;
}
