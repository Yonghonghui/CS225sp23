...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        {
            whenVisitedTable.insert(k, ll.date);
        }
        else if(whenVisitedTable.keyExists(k))
        {
            if (whenVisitedTable.find(k) < ll.date)
            {
                whenVisitedTable.remove(k);
                whenVisitedTable.insert(k,ll.date);
            }
        } 
    }      
                                  ...
                                  const string& url) const
{
     ...
     */
    string k=customer+url;
    if(whenVisitedTable.keyExists(k) == false)
    {
    
        return time_t();
    }
    else{return whenVisitedTable.find(k);} 
}
